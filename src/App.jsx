...
import { 
  ...
  FileSpreadsheet, LogOut, Shield, Users, Lock, AlertOctagon
} from 'lucide-react';
...
const FIELDS_CD = [
  'SHIPMENT', 'RETORNO DA OCORRÊNCIA', 'RESOLUÇÃO', 'CAUSA', 
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-50 text-slate-500 font-bold uppercase text-xs">
            <tr><th className="p-4">Email</th><th className="p-4">Função</th><th className="p-4">Entidade Vinculada (Ex: PATRUS)</th><th className="p-4">Ação</th></tr>
          </thead>
          <tbody>
            {users.map(u => (
              <tr key={u.id} className="border-t border-slate-100">
                ...
                <td className="p-4">
                  <select className="p-2 border rounded font-bold text-slate-600" defaultValue={u.role} id={`role-${u.id}`}>
                    <option value="TRP">Transportadora</option><option value="CD">CD / Regional</option><option value="ADMIN">Administrador</option>
                  </select>
                </td>
                <td className="p-4"><input type="text" className="p-2 border rounded w-full font-mono text-xs" defaultValue={u.assigned_entity} placeholder="Nome EXATO da TRP ou CD" id={`entity-${u.id}`}/></td>
                <td className="p-4"><button onClick={() => updateUser(u.id, document.getElementById(`role-${u.id}`).value, document.getElementById(`entity-${u.id}`).value)} className="bg-slate-900 text-white px-4 py-2 rounded-lg font-bold text-xs hover:bg-slate-700">Salvar</button></td>
              </tr>
            ))}
          </tbody>
        </table>
...
function App() {
  ...
  const fetchRecords = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('logistics_records').select('*').order('created_at', { ascending: false });
  ...
  useEffect(() => { if (profile) fetchRecords(); }, [profile]);

  const logAction = async (recordId, actionType, changedData) => {
    await supabase.from('audit_logs').insert([{
      record_id: recordId, action: actionType, changed_by: profile.email, details: changedData
    }]);
  };
  ...
  const handleDownloadTemplate = (type) => {
    ...
    let fileName = type === 'TRP' ? "Modelo_Apontamento_TRP.xlsx" : "Modelo_Resposta_CD.xlsx";
  ...
  const handleCreate = async () => {
    ...
    // Força a TRP a ser a entidade do perfil logado (Segurança)
    ...
    if (!error && data) {
      ...
      alert('Criado!'); setFormValues({}); fetchRecords(); setActiveTab('dashboard'); 
  ...
  const handleUpdate = async () => {
    ...
    if (!error) { 
      ...
      alert('Atualizado!'); setEditRecord(null); setFormValues({}); fetchRecords(); 
    } else { alert('Erro: ' + error.message); }
  };
  ...
  const handleBatchUpdate = (e) => {
    ...
    reader.onload = async (evt) => {
      ...
      for (let i = 0; i < excelRows.length; i++) {
        ...
        if (!row['SHIPMENT'] && !row['PEDIDO']) { validationError = `Linha ${i+2}: Falta SHIPMENT/PEDIDO`; break; }
        if (profile.role !== 'TRP') {
           for (const field of REQUIRED_FIELDS_RESPONSE) {
              if (!row[field]) { validationError = `Linha ${i+2}: Campo ${field} vazio.`; break; }
           }
        }
      ...
      if (validationError) {
        setProcessingBatch(false);
        alert("⛔ UPLOAD BLOQUEADO: " + validationError);
        return;
      }
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-20">
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in zoom-in-95">
          <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
            <div className="bg-orange-500 p-6 flex justify-between items-center text-white">
              <h3 className="font-bold text-xl flex items-center gap-2"><Edit /> Responder</h3>
            ...
            <div className="p-6 overflow-y-auto bg-slate-50">
              <div className="bg-white p-4 rounded-xl border border-orange-200 shadow-sm">
                ...
                {profile.role === 'CD' && renderFormFields(FIELDS_CD)}
                {profile.role === 'ADMIN' && renderFormFields(FIELDS_CD)} 
              </div>
            </div>
                 ...
                 <div className="space-y-3 max-h-40 overflow-y-auto pr-2">
                    {auditHistory.map(log => (
                      <div key={log.id} className="flex items-start gap-3 text-xs border-b border-slate-50 pb-2">
                         ...
                         <div><span className="font-bold text-slate-800">{log.action}</span> por <span className="bg-blue-100 text-blue-700 px-1 rounded font-bold">{log.changed_by}</span>
                         <div className="text-slate-400 mt-1 truncate w-64">{JSON.stringify(log.details)}</div></div>
                      </div>
                    ))}
                 </div>
              </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
             ...
             <p className="text-xs font-bold text-slate-400 mt-1">
               {profile.email} <span className="ml-2 bg-blue-100 text-blue-700 px-2 py-0.5 rounded uppercase">{profile.role} - {profile.assigned_entity || 'GLOBAL'}</span>
             </p>
          </div>
        </div>
        <div className="flex gap-2">
          {profile.role === 'ADMIN' && (
            <button onClick={() => setActiveTab('admin')} className="flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-sm bg-slate-100 hover:bg-slate-200"><Users size={16}/> Admin</button>
          )}
      ...
      {activeTab === 'admin' && profile.role === 'ADMIN' ? (
        ...
        <main className="max-w-7xl mx-auto p-6 mt-4">
            ...
            <div className="space-y-6 animate-in fade-in">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                 <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                    <p className="text-xs font-bold text-slate-400 uppercase">Total Visível</p>
                 ...
                 {/* BOTÕES PARA CD/ADMIN */}
                 {(profile.role === 'CD' || profile.role === 'ADMIN') && (
                   <div className="col-span-3 grid grid-cols-3 gap-2">
                      <button onClick={() => handleDownloadTemplate('CD')} className="bg-slate-100 text-slate-600 border border-slate-200 rounded-xl font-bold hover:bg-slate-200 transition-colors flex flex-col items-center justify-center py-2"><FileSpreadsheet size={20} className="mb-1"/> Modelo CD</button>
                      <button onClick={() => {
                      ...
                      }} className="bg-slate-800 text-white rounded-xl font-bold hover:bg-slate-700 transition-colors flex flex-col items-center justify-center py-2"><Download size={20} className="mb-1"/> Pendências</button>
                      <label className="cursor-pointer bg-orange-500 text-white rounded-xl font-bold hover:bg-orange-600 transition-colors flex flex-col items-center justify-center py-2 group">
                         <div className="flex items-center gap-2 mb-1"><Upload size={20}/></div> Subir Respostas
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <div className="p-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center gap-4">
                   <h3 className="font-bold text-slate-700 flex items-center gap-2"><LayoutDashboard size={18}/> Registros</h3>
                ...
                <div className="overflow-x-auto max-h-[600px]">
                  <table className="w-full text-left text-sm whitespace-nowrap">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {records.filter(r => JSON.stringify(r).toLowerCase().includes(searchTerm.toLowerCase())).map((r) => (
                        <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                          <td className="px-6 py-4 flex gap-2">
                            ...
                            {(profile.role === 'CD' || profile.role === 'ADMIN') && !r.retornoOcorrencia && (
          ...
          {/* VIEW: CREATE (TRP) */}
          {activeTab === 'create' && profile.role === 'TRP' && (
            <div className="max-w-4xl mx-auto space-y-6 animate-in slide-in-from-bottom-4">
              ...
              <div className="bg-white p-8 rounded-2xl shadow-lg border border-slate-100">
                ...
                <div className="flex justify-end mb-4">
                  <button onClick={() => handleDownloadTemplate('TRP')} className="flex items-center gap-2 text-xs font-bold text-blue-600 hover:bg-blue-50 px-3 py-2 rounded-lg border border-blue-100">
